function [con,Ime,Vme,Tmax,TimeLine]= ...
          FindResetTrans(n,delayTime,Ea,Rtop,Rbottom,Amplitude, ...
          threeD,ExportData)
% FindResetTrans
% --------------------------------------------------------------
...
%   ExportData - Record solutions in files
%   delayTime  - Time between two maximum values for thje electric 
%                potential.
%
% Outputs:
%   con        - Current simulation step counter when finished
%   Ime        - Vector of measured current values over time
%   Vme        - Vector of measured voltage values over time
%   Tmax       - Vector of maximum filament temperatures over time
%   TimeLine   - Vector of simulation times corresponding to each step
% --------------------------------------------------------------
...
model.param.set('timeDelay',strcat(num2str(delayTime),'[s]'));
...
logicalreset = true;  % Will remain true until rupture condition or n steps 
                      % reached.
Ime   = [0];          % Current measurements.
Vme   = [0];          % Voltage measurements.
Tmax  = [300];        % Max temperature (initial: room temperature).
con   = 1;            % Step counter.
time  = 0;            % Current simulation time.
timeSample = 5.0e-8;  % Time increment per step [s].
TimeLine = [0];       % Time points for each simulation.
...
while (logicalreset)
    ...
    if (logicalreset)
        % Maximum temperature from table 'tbl2'. What we do is take the 
        % domain corresponding to the entire filament and find its maximum 
        % temperature. This is done over time, taking the last temporal 
        ...
        model.result.numerical('max1').setIndex('interp', 'TimeMax', 0);
        model.result.numerical('max1').setResult;
        tempTmax = model.result.table('tbl2').getReal;
        Tmax = [Tmax tempTmax(2)];
        ...
        % being subjected to the source.
        model.result.numerical('av1').set('table', 'tbl3');
        model.result.table('tbl3').clearTableData;
        model.result.numerical('av1').setIndex('interp', 'TimeMax', 0);
        model.result.numerical('av1').setResult;
        tempVcell = model.result.table('tbl3').getReal;
        Vme = [Vme tempVcell(2)];
        ...
        % through the cell for each voltage pulse supplied to it.
        model.result.numerical('int1').set('table', 'tbl4');
        model.result.table('tbl4').clearTableData;
        model.result.numerical('int1').setIndex('interp', 'TimeMax', 0);
        model.result.numerical('int1').setResult;
        Itemp = model.result().numerical('int1').getReal;        
        Ime=[Ime Itemp];
        ...
        % If stopped, set final current to 0 and voltage slightly above last
        Ime=[Ime 0];
        Vme=[Vme Vme(con-1)+1.0e-3];
    end % End of logicalreset

end  % End of while loop

% ===============================================================
% Export of simulated signals to plain-text files (space-separated)
%
% We export three two-column text files (time vs. signal):
%   1) time vs. integral of current density through the filament (Ime)
%   2) time vs. maximum filament temperature (Tmax)
%   3) time vs. applied voltage at the metallization (Vme)
%
% Each file is written as plain text, with values separated by a
% single space (no commas), suitable for visualization or post-processing.
% Before writing, we verify that the time vector (TimeLine) and the
% corresponding signal vector have the same length.
% ===============================================================

if (ExportData)

    % --- Export: time vs. maximum filament temperature (Tmax) ---
    if numel(TimeLine) == numel(Tmax)
        fileName = strcat(pathCal,'current/time_VS_TMax.txt');
        writematrix([TimeLine(:) Tmax(:)], fileName, 'Delimiter', ...
            ' ', 'FileType', 'text');
    else
        error('TimeLine and Tmax must have the same number of elements.');
    end

    % In order to get the same length between TimeLine and Ime as well as 
    % Vme we add a step of time more
    newtime = TimeLine(numel(TimeLine))+TimeLine(2)-TimeLine(1);
    TimeLine = [TimeLine newtime];

    % --- Export: time vs. integral of current density (Ime) ---
    if numel(TimeLine) == numel(Ime)
        fileName = strcat(pathCal,'current/time_VS_I.txt');
        writematrix([TimeLine(:) Ime(:)], fileName, 'Delimiter', ...
            ' ', 'FileType', 'text');
    else
        error('TimeLine and Ime must have the same number of elements.');
    end

    % --- Export: time vs. applied voltage at the metallization (Vme) ---
    if numel(TimeLine) == numel(Vme)
        fileName = strcat(pathCal,'current/time_VS_V.txt');
        writematrix([TimeLine(:) Vme(:)], fileName, 'Delimiter', ...
            ' ', 'FileType', 'text');
    else
        error('TimeLine and Vme must have the same number of elements.');
    end

    % --- Smooth maximum temperature vs. time ---
infile = strcat(pathCal,'current/time_VS_TMax.txt');
if isfile(infile)
    outfile = strcat(pathCal,'current/tTMax.txt');
    smooth_data(infile, outfile, 5, 'exact');
end

% --- Smooth applied voltage vs. time ---
infile = strcat(pathCal,'current/time_VS_V.txt');
if isfile(infile)
    outfile = strcat(pathCal,'current/tV.txt');
    smooth_data(infile, outfile, 5, 'exact');
end

% --- Smooth integrated current density vs. time ---
infile = strcat(pathCal,'current/time_VS_I.txt');
if isfile(infile)
    outfile = strcat(pathCal,'current/tI.txt');
    smooth_data(infile, outfile, 5, 'exact');
end
end
...
%% Example call:
% [con,Ime,Vme,Tmax,TimeLine]=FindResetTrans(990,7.5E-6,1.08,5,2.5,1.0e7,
% false,true)
