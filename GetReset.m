...
% ===============================================================
% Post-processing: smoothing of simulated data
%
% The following steps apply smoothing to the exported simulation
% results of a resistive random-access memory cell.
% Due to small numerical fluctuations during the transient 
% COMSOL simulations, the raw data may contain noise.
%
% We smooth the following quantities in blocks of 50 samples:
%   - Maximum filament temperature (Tmax)
%   - Applied voltage (Vapp)
%   - Integrated current density (IntJz)
%
% This smoothing makes the results clearer and easier to
% visualize, highlighting the physical trends rather than 
% numerical artifacts.
% ===============================================================

% --- Smooth maximum temperature vs. time ---
infile = strcat(pathCal,'current/timeTMax.txt');
if isfile(infile)
    outfile = strcat(pathCal,'current/time_VS_TMax.txt');
    smooth_data(infile, outfile, 50, 'exact');
end

% --- Smooth applied voltage vs. time ---
infile = strcat(pathCal,'current/timeVapp.txt');
if isfile(infile)
    outfile = strcat(pathCal,'current/time_VS_V.txt');
    smooth_data(infile, outfile, 50, 'exact');
end

% --- Smooth integrated current density vs. time ---
infile = strcat(pathCal,'current/timeIntJz.txt');
if isfile(infile)
    outfile = strcat(pathCal,'current/time_VS_I.txt');
    smooth_data(infile, outfile, 50, 'exact');
end
